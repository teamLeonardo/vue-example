    ...
    <div class="dropdown dropdown-end">
        <div tabindex="0" role="button" class="btn btn-ghost btn-circle">
            <div class="indicator">
                ...
                <span class="badge badge-sm indicator-item">{{ store.data.length }}</span>
            </div>
        </div>
        <div tabindex="0" class="mt-3 z-[1] card card-compact dropdown-content w-52 bg-base-100 shadow">
            <div class="card-body">
                <span class="font-bold text-lg">{{ store.data.length }} Items</span>
                <span class="text-info">Subtotal: {{ store.total }}</span>
                <div class="card-actions">
                    <button @click="() => $router.push(PATH.CART)" class="btn btn-primary btn-block">
                        View cart
                    </button>
                </div>
            </div>
